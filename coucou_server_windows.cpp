...
#define CLOSE_SOCKET(s) CLOSE_SOCKET(s)
#define CLOSE_SOCKET(s) close(s)
#endif

#ifdef _WIN32
// Windows에 ssize_t가 없으므로 int로 매핑
typedef int ssize_t;
#endif
...
void send_json(socket_t client_sock, const json& response) {
...
MYSQL* connect_db() {
    ...
    if (!mysql_real_connect(conn, "10.10.20.105", "USER", "1234", "apayo", 0, NULL, 0)) {
...
/////////////////////////////////////
void handle_json_message(const std::string& message, socket_t client_sock) {
...
// 클라이언트 요청 처리 스레드
/////////////////////////////////////
void client_worker(socket_t client_sock, std::string client_ip) {
    char buffer[2048];
...
void start_server() {
    ...
    while (true) {
        ...
        socket_t client_sock = accept(server_fd, (struct sockaddr*)&client_addr, &client_len);
